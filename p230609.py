...
for i in range(skiki):
 perevirka(i,dx,0)
 print("Right shifting cluster number ",i)
 perevirka(i,-dx,0)
 print("Left shifting cluster number ",i)
 perevirka(i,0,dy)
 print("Up shifting cluster number ",i)
 perevirka(i,0,-dy)
 print("Down shifting cluster number ",i)
p[n1][0]=p[n1][0]+dx1
p[n1][1]=p[n1][1]+dy1
print(n1, p[n1][0],p[n1][1])
# Plotting
...
for i in range(len(conn)):
 n1=conn[i][0]
 n2=conn[i][1]
 plt.plot((p[n1][0],p[n2][0]),(p[n1][1],p[n2][1]),color='green')
plt.xlabel('RA')
plt.ylabel('DEC')
plt.ylim(-10,70)
